    ...
    CS: ConstraintSystem<Scalar>,
{
    ...
    for (i, block) in padded.chunks(512).enumerate() {
        cur = cs.namespace(
            || format!("block {}", i),
            |ns| sha256_compression_function(ns, block, &cur),
        )?;
    }
    ...
    CS: ConstraintSystem<Scalar>,
{
    ...
    for i in 16..64 {
        cs.namespace(
            ...
            |mut cs| {
                ...
                let tmp = cs.namespace(
                    || "computation of w[i]",
                    |ns| UInt32::addmany(ns, &[w[i - 16].clone(), s0, w[i - 7].clone(), s1]),
                )?;
    ...
    for i in 0..64 {
        (a, b, c, d, e, f, g) = cs.namespace(
            || format!("compression round {}", i),
            |mut cs| {
    ...
    */
    ...
    let h0 = cs.namespace(
        || "deferred h0 computation",
        |ns| a.compute(ns, &[current_hash_value[0].clone()]),
    )?;

    let h1 = cs.namespace(
        || "new h1",
        |ns| UInt32::addmany(ns, &[current_hash_value[1].clone(), b]),
    )?;

    let h2 = cs.namespace(
        || "new h2",
        |ns| UInt32::addmany(ns, &[current_hash_value[2].clone(), c]),
    )?;

    let h3 = cs.namespace(
        || "new h3",
        |ns| UInt32::addmany(ns, &[current_hash_value[3].clone(), d]),
    )?;

    let h4 = cs.namespace(
        || "deferred h4 computation",
        |ns| e.compute(ns, &[current_hash_value[4].clone()]),
    )?;

    let h5 = cs.namespace(
        || "new h5",
        |ns| UInt32::addmany(ns, &[current_hash_value[5].clone(), f]),
    )?;

    let h6 = cs.namespace(
        || "new h6",
        |ns| UInt32::addmany(ns, &[current_hash_value[6].clone(), g]),
    )?;

    let h7 = cs.namespace(
        || "new h7",
        |ns| UInt32::addmany(ns, &[current_hash_value[7].clone(), h]),
    )?;
...
mod test {
    ...
    fn test_full_block() {
        ...
        let input_bits: Vec<_> = (0..512)
            .map(|i| {
                Boolean::from(
                    cs.namespace(
                        || format!("input bit {}", i),
                        |ns| AllocatedBit::alloc(ns, Some(rng.next_u32() % 2 != 0)),
                    )
                    .unwrap(),
                )
            })
        ...
        cs.namespace(
            || "sha256",
            |ns| sha256_compression_function(ns, &input_bits, &iv),
        )
        .unwrap();
    ...
    fn test_full_hash() {
        ...
        cs.namespace(|| "sha256", |ns| sha256(ns, &input_bits))
            .unwrap();
